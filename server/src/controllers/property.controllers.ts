...
export const getProperties = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (availableFrom && availableFrom !== "any") {
            ...
            if (availableFromData) {
                const date = new Date(availableFromData);
...
export const getProperty = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (property) {
            const coordinates: { cordinates: string }[] = await prisma.$queryRaw`
                SELECT ST_asText(coordinates) as cordinates
                FROM "Location"
                WHERE id = ${property.location.id}
            `;

            const geoJson: any = wktToGeoJSON(coordinates[0]?.cordinates || "");
